...
- Selector and envelope comparison colors are now synchronized.
- The automatic peak selection button is now named **Detectar Automático**.
- Envelope clicks are faster because dominant peak detection is cached and the selector image uses a capped rendering point count.
- The **Potência** operation is faster because expensive delay/FFT calculations use a bounded representative sample, while full-resolution energy, charge and extrema are preserved.
- Full V-I-P CSV generation is now prepared on demand.
